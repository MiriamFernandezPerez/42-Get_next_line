...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/02/16 20:12:34 by mirifern          #+#    #+#             */
/*   Updated: 2024/03/02 00:43:46 by mirifern         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_clean_buffer(char *buffer)
{
	...
	new_buf = ft_calloc(sizeof(char), ft_strlen(buffer));
	if (!new_buf)
	{
		free(buffer);
		return (NULL);
...
char	*ft_make_line(char *buffer)
{
	...
	if (buffer[i] == '\n')
		bits = 2;
	line = ft_calloc(sizeof(char), (i + bits));
	if (!line)
		return (NULL);
	i = 0;
	while (buffer[i] != '\0' && buffer[i] != '\n')
